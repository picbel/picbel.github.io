...
- 앤티티에서 사용할 고유 Id를 지정하는 어노테이션
  ...
  - 추후 Spring Data Jpa와의 사용성을 생각한다면 Object type으로 선언하는 것을 권장한다. 기본 타입일 경우 null 체크에 애로 사항이 생긴다.\
    int memberId가 있다 했을 때 값이 0일 경우 이게 진짜 0이라서 그런 건지 기본타입이라 값이 없어 0인지 확인 할 수가 없다.
